...
// 1. 定义测速目标配置
const TEST_URLS = [
  { label: 'Google', value: 'https://www.google.com/generate_204', icon: <GoogleOutlined /> },
  { label: 'YouTube', value: 'https://www.youtube.com/generate_204', icon: <YoutubeOutlined /> },
...
const HomePage: React.FC = () => {
  ...
  // ✅ 2. 补全状态：记录当前选择的测速 URL
  const [testUrl, setTestUrl] = useState(TEST_URLS[0].value); 
  const [urlDelay, setUrlDelay] = useState<number | null>(null); // 记录 URL 测速结果
  ...
  const sortedNodes = useMemo(() => {
    ...
    if (sortType === "name") {
      ...
      list.sort((a, b) => {
        ...
        const valA = latA === -1 || latA === undefined ? Number.MAX_SAFE_INTEGER : latA;
        const valB = latB === -1 || latB === undefined ? Number.MAX_SAFE_INTEGER : latB;
        return valA - valB;
      });
    }
    return list;
  ...
  const handleSpeedTest = async () => {
    ...
    message.success({ content: `测速完成，${successCount} 个节点可用`, key: 'test' });
  };

  // ✅ 新增：针对特定 URL 的真连接测速 (仅在已连接时有效)
  const handleUrlConnectivityTest = async () => {
    ...
    message.loading({ content: `正在通过代理访问 ${targetLabel}...`, key: 'urltest' });
    setUrlDelay(null);

    const start = performance.now();
    try {
        // 利用浏览器 fetch，它会自动走系统代理
        await fetch(testUrl, { method: 'HEAD', mode: 'no-cors', cache: 'no-cache' });
        const ms = Math.round(performance.now() - start);
        
        setUrlDelay(ms);
        message.success({ content: `${targetLabel} 连接成功: ${ms}ms`, key: 'urltest' });
    } catch (e) {
        setUrlDelay(-1);
      ...
      <div className="bg-white px-6 py-4 shadow-sm border-b border-gray-100 relative z-10">
        ...
        {isRunning && activeNode ? (
          <div className="mb-4 bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-center justify-between animate-in slide-in-from-top-2 duration-300">
            ...
            {/* 显示 URL 测速结果 */}
            {urlDelay !== null && (
                <Tag color={urlDelay === -1 ? "error" : "success"} className="ml-auto mr-4 text-base px-3 py-1">
                    {currentTestUrlObj?.label}: {urlDelay === -1 ? "Timeout" : `${urlDelay}ms`}
                </Tag>
            )}

            <Button danger type="primary" shape="round" icon={<PoweroffOutlined />} onClick={toggleProxy}>
              断开
            </Button>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-2 mb-4">
            <div onClick={toggleProxy} className="w-20 h-20 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center cursor-pointer transition-all text-gray-400 hover:text-gray-600 mb-2">
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <Space>
            {/* 测速目标选择 & 连通性测试 */}
            <Dropdown.Button 
                onClick={handleUrlConnectivityTest} // 点击左侧按钮：测试连通性
                menu={{ 
            ...
            <Button icon={<ThunderboltOutlined />} loading={isTesting} onClick={handleSpeedTest}>
              批量延迟
            </Button>
