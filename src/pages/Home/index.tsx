...
const Home = () => {
  ...
  const startProxy = async () => {
    const msg = await invoke("start_singbox", { configName: "config.json", port: 1080 });
    setStatus(msg as string);
  };

  const stopProxy = async () => {
    const msg = await invoke("stop_singbox");
    setStatus(msg as string);
  };

  return (
    <div>
      <button onClick={startProxy}>启动 Sing-box 并开启代理</button>
      <button onClick={stopProxy}>停止 Sing-box 并关闭代理</button>
      <p>{status}</p>
    </div>
  );
}
